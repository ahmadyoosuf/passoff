...
const MODES: { value: PlaygroundMode; label: string; description: string }[] = [
  {
    value: "raw",
    label: "Raw",
    description: "Writer receives raw researcher output",
  },
  {
    value: "passoff",
    label: "PASSOFF",
    description: "Writer receives a PASSOFF Briefing",
  },
  {
    value: "ab",
    label: "A/B",
    description: "Side-by-side comparison of both",
  },
];

export default function PipelineRunner() {
  const [topic, setTopic] = useState(
    "LLM multi-agent coordination failures"
  );
  ...
  return (
    <div className="flex flex-col gap-6 p-4 lg:p-6 max-w-6xl mx-auto w-full">
      {/* Header */}
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1 leading-relaxed">
          Run a two-agent pipeline (Researcher then Writer) and compare
          outcomes with and without PASSOFF.
        </p>
      </div>
      ...
      <div className="flex items-start gap-3 border border-border/30 rounded-lg p-3 bg-muted/20">
        ...
        <p className="text-xs text-muted-foreground leading-relaxed">
          On the three validated scenarios, PASSOFF outperforms raw context on
          all three based on pre-committed outputs. On live custom topics, you
          are running a real experiment and results vary. That variance is the
          point -- it means the scores are real.
        </p>
      </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          {MODES.map((m) => (
            <button
              ...
              className={cn(
                "px-3 py-2 rounded-md text-xs font-mono transition-colors touch-target-sm",
                mode === m.value
          ...
          <Button
            ...
            className="gap-2 flex-1 sm:flex-none sm:min-w-[100px]"
          >
            {loading ? (
      ...
      {result && (
        <div className="space-y-6 animate-fade-in-up">
          ...
          {result.passoffBriefing &&
            (mode === "passoff" || mode === "ab") && (
              <div className="space-y-2">
                <div className="flex items-center gap-2">
                    ...
                    {result.passoffBriefing.decisions.length > 0 && (
                      <div>
                        ...
                        {result.passoffBriefing.decisions.map((d, i) => (
                          <p
                            key={i}
                            className="text-xs text-foreground ml-2"
                          >
                            {">"} {d.constraint}
                          </p>
                        ))}
